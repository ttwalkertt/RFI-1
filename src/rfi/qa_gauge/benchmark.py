...
class BenchmarkCorpus:
    ...
    def load(cls, root: Path) -> BenchmarkCorpus:
        ...
        cases = tuple(
            json.loads(line)
            for line in (resolved / "candidate_cases.jsonl").read_text(encoding="utf-8").splitlines()
            if line.strip()
        )
    ...
    def borderline(self) -> tuple[dict[str, Any], ...]:
        return tuple(case for case in self.cases if case["adjudicability"]["status"] == "borderline")

    def reviewer_payload(self, case_id: str) -> dict[str, Any]:
        """Project only ticket-authorized input; hidden reference fields never cross this boundary."""
...
def prepare_control_manifests(
    *,
    ...
    for name in ("corpus-freeze-manifest.json", "quarantine-manifest.json", "partition-manifest.json"):
        if (output / name).exists():
            raise ValueError("control manifests already exist; partition regeneration is prohibited")
    ...
    quarantine = {
        ...
        "reason": "judgment-dependent; excluded unchanged from calibration, validation, scoring, RBF feedback, and success criteria",
        "cases": [
            {
    ...
    partition = {
        ...
        "seed": seed,
        "algorithm": "sort pair IDs; Python random.Random(seed).shuffle; first 20 calibration, remaining 11 validation",
        "unit": "matched pair",
        "rerandomized": False,
        "rejected_splits": [],
        "calibration": {
        ...
        "held_out_protection": "validation payloads, labels, outputs, and scores are inaccessible to run-calibration; run-validation requires a matching frozen-gauge manifest and one-shot marker",
    }
    for name, payload in (
