#!/usr/bin/env python3
"""Validate independent RFI QA benchmark v3, including protected split digests."""
...
VERSION = "rfi-qa-independent-v3.0.0"
PARTITIONS = ("calibration", "verification", "quarantine")
OBJECTIVE_PARTITIONS = ("calibration", "verification")
CASE_KEYS = {
...
def validate_schema_documents(taxonomy_classes: set[str]) -> None:
    ...
    if not re.fullmatch(case_schema["properties"]["case_id"]["pattern"], "R3C-001"):
        fail("case schema does not accept objective IDs")
    if not re.fullmatch(case_schema["properties"]["case_id"]["pattern"], "R3Q-001"):
...
def read_fixtures(partition: str) -> dict[str, dict[str, Any]]:
    ...
    for fixture in collection["fixtures"]:
        ...
        expected_fixture_pattern = r"R3F-(?:[0-9]{3}|Q[0-9]{3})"
        ...
        if authority["locator"] != f"rfiqa-v3://{fixture_id}/@authority":
        ...
        for record in fixture["records"]:
            ...
            expected_prefix = f"rfiqa-v3://{fixture_id}/{evidence_id}#"
...
def validate_case(
    ...
    if not re.fullmatch(r"R3(?:C|Q)-[0-9]{3}", case_id):
    ...
    if partition == "quarantine":
        if not case_id.startswith("R3Q-") or case["pair_id"] is not None:
        ...
        if not case_id.startswith("R3C-") or not re.fullmatch(r"R3P-[0-9]{3}", case["pair_id"] or ""):
...
def validate_file_digests(manifest: dict[str, Any], full: bool) -> None:
    ...
    for relative, expected in expected_files.items():
        ...
        elif full or not relative.startswith("verification/"):
    ...
    for partition in PARTITIONS:
        ...
        if not cases_path.exists() and partition == "verification" and not full:
            continue
...
def validate_manifest_privacy(manifest: dict[str, Any]) -> None:
    ...
    prohibited_fragments = ("R3C-", "R3Q-", "rfiqa-v3://", "investigation_question", "material_findings", "submitted_answer")
    ...
    if confidentiality["manifest_verification_metadata_is_aggregate_only"] is not True:
...
def main() -> None:
    ...
    verification_paths = (ROOT / "verification" / "cases.jsonl", ROOT / "verification" / "fixtures.json")
    verification_present = all(path.exists() for path in verification_paths)
    verification_any = any(path.exists() for path in verification_paths)
    full = args.full or (not args.visible and verification_present)
    if args.full and not verification_present:
        fail("--full requested but verification partition is absent")
    if args.visible and verification_any:
        fail("--visible requires held-out verification files to be physically absent")
    ...
    population = [f"R3P-{number:03d}" for number in range(split["objective_pair_id_population"]["first"], split["objective_pair_id_population"]["last"] + 1)]
    ...
    expected_verification = set(randomized[split["calibration_pair_target"] :])
    ...
    if set_digest(expected_verification) != manifest["partitions"]["verification"]["pair_set_sha256"]:
        fail("verification pair-set digest mismatch")

    if full:
        actual_verification = {case["pair_id"] for case in loaded["verification"][0]}
        if actual_verification != expected_verification:
            fail("verification pair assignment does not reproduce from seed")
        ...
        objective_cases = loaded["calibration"][0] + loaded["verification"][0]
        objective_summary = {
        ...
        verification = manifest["partitions"]["verification"]
        combined_dispositions = Counter(calibration["disposition_counts"]) + Counter(verification["disposition_counts"])
        if {
            "pair_count": calibration["pair_count"] + verification["pair_count"],
            "case_count": calibration["case_count"] + verification["case_count"],
    ...
    print("verification aggregate:", manifest["partitions"]["verification"])
