#!/usr/bin/env python3
"""Generate or independently verify the commit-aware TASK-058 review package."""
...
PACKAGE = ROOT / ".artifacts/review/TASK-058"
ZIP_PATH = ROOT / ".artifacts/review/TASK-058-review.zip"
VALIDATION = ROOT / ".artifacts/task058-validation"
...
def generate(base: str | None) -> int:
    outcomes = [
        run("focused", ["make", "task058-test"]),
        run("manual-proof", ["make", "task058-proof"]),
    ...
    if any(item["exit_code"] for item in outcomes):
        raise RuntimeError("one or more TASK-058 validations failed")
    copied = [
        (
            "task-ticket.md",
            ROOT / "tasks/TASK-058-Externalize-Transcript-Acquisition-Orchestration.md",
        ),
        (
            "design/TRANSCRIPT_LLM_SEED_RECOVERY.md",
            ROOT / "docs/design/TRANSCRIPT_LLM_SEED_RECOVERY.md",
        ),
        ("completion/repository-review.md", ROOT / "docs/TASK-058-review.md"),
        ("evidence/task058-tests.py", ROOT / "tests/test_task058.py"),
        (
            "evidence/task058-observable-compatibility.py",
            ROOT / "scripts/task058_orchestration.py",
        ),
    ...
    build_package(
        root=ROOT, task_id="TASK-058", package=PACKAGE, zip_path=ZIP_PATH,
        copied_members=copied, base_ref=base,
    )
    report = verify_package(ZIP_PATH, expected_task_id="TASK-058")
...
def main() -> int:
    ...
    if args.verify:
        print(json.dumps(
            verify_package(args.verify, expected_task_id="TASK-058"), indent=2
        ))
        return 0
