#!/usr/bin/env python3
"""Generate or independently verify the commit-aware TASK-057 review package."""
...
PACKAGE = ROOT / ".artifacts/review/TASK-057"
ZIP_PATH = ROOT / ".artifacts/review/TASK-057-review.zip"
VALIDATION = ROOT / ".artifacts/task057-validation"
...
def generate(base: str | None) -> int:
    outcomes = [
        run("focused", ["make", "task057-test"]),
        run("reproduction", ["make", "task057-proof"]),
    ...
    if any(item["exit_code"] for item in outcomes):
        raise RuntimeError("one or more TASK-057 validations failed")
    copied = [
        (
            "task-ticket.md",
            ROOT / "tasks/TASK-057-harden-transcript-discovery-traversal-and-diagnostics.md",
        ),
        ("completion/repository-review.md", ROOT / "docs/TASK-057-review.md"),
        ("evidence/task057-tests.py", ROOT / "tests/test_task057.py"),
        ("evidence/reproduction.json", VALIDATION / "reproduction.json"),
        ("validation/focused.txt", VALIDATION / "focused.txt"),
        ("validation/reproduction.txt", VALIDATION / "reproduction.txt"),
    ...
    build_package(
        root=ROOT, task_id="TASK-057", package=PACKAGE, zip_path=ZIP_PATH,
        copied_members=copied, base_ref=base,
    )
    report = verify_package(ZIP_PATH, expected_task_id="TASK-057")
...
def main() -> int:
    ...
    if args.verify:
        print(json.dumps(verify_package(args.verify, expected_task_id="TASK-057"), indent=2))
        return 0
