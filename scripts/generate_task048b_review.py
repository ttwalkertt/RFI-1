#!/usr/bin/env python3
"""Generate and verify the self-contained TASK-048B review package."""
...
PACKAGE = ROOT / ".artifacts" / "review" / "TASK-048B"
ZIP_PATH = PACKAGE.parent / "TASK-048B-review.zip"
...
def main() -> int:
    ...
    validations = [
        run("focused", [
            sys.executable, "-m", "unittest", "tests.test_task041",
            "tests.test_source_profile_digest_compatibility", "tests.test_task014",
            "tests.test_task048a", "-v",
        ]),
    ...
    for destination, source in {
        "task-ticket.md": ROOT / "tasks/TASK-048B-source-profile-startup-compatibility.md",
        "completion/repository-review.md": ROOT / "docs/TASK-048B-review.md",
        "evidence/digest-compatibility-tests.py": (
    ...
    write(
        "manifest.json",
        json.dumps(
            {"task": "TASK-048B", "members": {
                member: sha256(PACKAGE / member) for member in members
            }},
            indent=2,
        ) + "\n",
    )
    ...
    if bad_member is not None:
        raise RuntimeError(f"review package member failed integrity: {bad_member}")
