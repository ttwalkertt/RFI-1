#!/usr/bin/env python3
"""Generate and verify the self-contained TASK-047 review package."""
...
PACKAGE = ROOT / ".artifacts" / "review" / "TASK-047"
ZIP_PATH = PACKAGE.parent / "TASK-047-review.zip"
...
def main() -> int:
    ...
    validations = [
        run("focused-task047", [sys.executable, "-m", "unittest", "tests.test_task047", "-v"]),
        run(
            "acquisition-repository-regression",
            [
                sys.executable, "-m", "unittest", "tests.test_acquisition",
                "tests.test_task021", "-v",
            ],
        ),
    ...
    for destination, source in {
        "task-ticket.md": ROOT / "tasks" / "TASK-047 — Define the Date-Delimited Acq.md",
        "architecture/contract.md": ROOT / "docs" / "date-delimited-acquisition-contract.md",
        "architecture/concept-of-operation.md": ROOT / "docs" /
            "Non-deterministic Document Fetch Concept of Operation.md",
        "completion/repository-review.md": ROOT / "docs" / "TASK-047-review.md",
        "evidence/test_task047.py": ROOT / "tests" / "test_task047.py",
    }.items():
    ...
    write("manifest.json", json.dumps({"task": "TASK-047", "members": checksums}, indent=2) + "\n")
    ...
    expected = sorted(
        f"TASK-047/{path.relative_to(PACKAGE).as_posix()}"
