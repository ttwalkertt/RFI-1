#!/usr/bin/env python3
"""Generate or independently verify the commit-aware TASK-059 review package."""
...
PACKAGE = ROOT / ".artifacts/review/TASK-059"
ZIP_PATH = ROOT / ".artifacts/review/TASK-059-review.zip"
VALIDATION = ROOT / ".artifacts/task059-validation"
...
def generate(base: str | None) -> int:
    outcomes = [
        run("focused", ["make", "task059-test"]),
        run("manual-proof", ["make", "task059-proof"]),
        run("task058-regression-proof", ["make", "task058-proof"]),
    ...
    if any(item["exit_code"] for item in outcomes):
        raise RuntimeError("one or more TASK-059 validations failed")
    copied = [
        (
            "task-ticket.md",
            ROOT / "tasks/TASK-059-Add-Explicit-Transcript-Acquisition-Selection-Criteria.md",
        ),
        (
            "design/TRANSCRIPT_LLM_SEED_RECOVERY.md",
            ROOT / "docs/design/TRANSCRIPT_LLM_SEED_RECOVERY.md",
        ),
        ("completion/repository-review.md", ROOT / "docs/TASK-059-review.md"),
        ("evidence/task059-tests.py", ROOT / "tests/test_task059.py"),
        ("evidence/task059-selection-proof.py", ROOT / "scripts/task059_selection.py"),
        ("validation/focused.txt", VALIDATION / "focused.txt"),
        ("validation/manual-proof.txt", VALIDATION / "manual-proof.txt"),
        (
            "validation/task058-regression-proof.txt",
            VALIDATION / "task058-regression-proof.txt",
        ),
    ...
    build_package(
        root=ROOT, task_id="TASK-059", package=PACKAGE, zip_path=ZIP_PATH,
        copied_members=copied, base_ref=base,
    )
    report = verify_package(ZIP_PATH, expected_task_id="TASK-059")
...
def main() -> int:
    ...
    if args.verify:
        print(json.dumps(
            verify_package(args.verify, expected_task_id="TASK-059"), indent=2
        ))
        return 0
