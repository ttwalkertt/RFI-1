#!/usr/bin/env python3
"""Generate or independently verify the commit-aware TASK-055 review package."""
...
PACKAGE = ROOT / ".artifacts/review/TASK-055"
ZIP_PATH = ROOT / ".artifacts/review/TASK-055-review.zip"
VALIDATION = ROOT / ".artifacts/task055-validation"
...
def generate(base: str | None) -> int:
    outcomes = [
        run_validation("focused", ["make", "task055-test"]),
        run_validation("operator-proof", ["make", "task055-proof"]),
    ...
    if any(item["exit_code"] for item in outcomes):
        raise RuntimeError("one or more TASK-055 validations failed")
    copied = [
        (
            "task-ticket.md",
            ROOT / "tasks/TASK-055-detect-unimported-external-firm-configuration-v2.md",
        ),
        ("completion/repository-review.md", ROOT / "docs/TASK-055-review.md"),
        ("evidence/task055-tests.py", ROOT / "tests/test_task055.py"),
        ("evidence/operator-proof.py", ROOT / "scripts/task055_configuration_status.py"),
    ...
    report = verify_package(ZIP_PATH, expected_task_id="TASK-055")
...
def main() -> int:
    ...
    if args.verify:
        print(json.dumps(
            verify_package(args.verify, expected_task_id="TASK-055"), indent=2
        ))
        return 0
