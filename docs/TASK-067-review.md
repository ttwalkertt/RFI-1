...
dismiss/restore, upload and alternate-URL fulfillment, and aggregate RSS export.
...
owned by cron, launchd, systemd timers, or another external scheduler.
...
pull inclusion, CLI, APIs, and valid RSS 2.0.
...
- Public URL screening rejects credentials, literal private/loopback/link-local addresses, and
  ...
  service.
- A process crash can leave a durable `running` feed row that intentionally blocks later overlap;
  a future operator recovery action should reconcile abandoned runs if operational evidence shows
  this is needed.
...
| Manual fulfillment | Upload/alternate candidate qualification and linking | Complete | Original failure history is preserved |
| Feed-run instrumentation | Durable authoritative structured results | Complete | Newest 100 retained; expandable UI uses exact JSON |
| Aggregate RSS | Offline bounded projection of durable observations | Complete | 200 items; retained/unavailable status explicit |
...
The next architectural milestone is operational recovery and scheduling policy only if real use
demonstrates the need; it is not a second feed-processing path.
