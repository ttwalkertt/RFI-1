...
Lore/public-inbox profiles are repository-global objects, not firm-owned objects. Streams consume
existing repository-provided governed sources; `/firms` remains the separate firm-owned
acquisition configuration surface. A saved Lore identity and its transport policy are immutable so
existing acquisition and stream references retain their meaning.
